...
An iPhone and Android app that acts as a remote control for Sony Bravia TVs (and any Android TV — Shield, Chromecast with Google TV, etc.) over your local WiFi.

Now features a stunning **Aurora Glass redesign**, local **mDNS automatic TV discovery** (no IP typing required!), and **direct streaming app launching tiles** (Netflix, YouTube, Prime Video) deep-linked directly to the TV.

No cloud server, no account, no special TV settings. The phone talks directly to the TV.

## How it works in plain English

1. **Automatic Discovery.** The app automatically browses the local Wi-Fi network for compatible Android TV/Bravia devices using mDNS.
2. **One-time pairing.** Tap your TV from the cast picker. Your phone introduces itself to the TV. The TV displays a code on screen, you type it into the app. They exchange digital certificates and remember each other.
3. **Daily use.** Tap a button or streaming tile → app sends a secure, framed protobuf message over WiFi → TV reacts. Volume, channel, D-pad, power, and apps launch instantly.
...
├── modules/
│   ├── atv-cert/                    JS side of the native TLS module (Port 6466/6467)
│   └── atv-discovery/               JS side of the native mDNS Bonjour/NSD module
└── src/
    ...
    │   └── atvDiscovery.ts          JS facade of the discovery store & hook
    ├── components/
    │   ├── RemoteButton.tsx         Haptics + debounce + hold-repeat
    │   ├── GlassButton.tsx          Frosted glass remote button
    │   ├── AuroraBackground.tsx     Soft gradient backdrop
    │   ├── BottomSheet.tsx          Frosted glass slide-up drawer
    │   ├── TVPickerSheet.tsx        Selects discovered TVs on local Wi-Fi
    │   ├── StreamingTile.tsx        Deep-links to Netflix, YouTube, Prime
    │   ├── DPad.tsx                 Restyled glass arrows and gradient OK button
    │   ├── Rocker.tsx               Restyled glass rocker switches
    │   ├── NumberPad.tsx
    │   └── Toast.tsx
    ├── context/
    │   └── SettingsContext.tsx      Persists IP, client cert, and discovery info
    ├── screens/
    │   ├── RemoteScreen.tsx         Frosted glass Remote interface
